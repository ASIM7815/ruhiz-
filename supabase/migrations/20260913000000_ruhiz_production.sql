...
-- If an older Supabase starter schema used profiles.id directly as auth.users.id,
-- preserve that account link in the new user_id column.
...
set username = coalesce(
  nullif(regexp_replace(username, '[^a-zA-Z0-9_]', '', 'g'), ''),
  'member_' || left(id::text, 8)
)
where username is null or username !~ '^[a-zA-Z0-9_]+$';
...
exception when others then
  raise notice 'profiles username/display_name still contain nulls: %', sqlerrm;
end $$;
...
exception when unique_violation then
  raise notice 'profiles username lower-case uniqueness needs manual duplicate cleanup: %', sqlerrm;
end $$;
...
-- add username shape constraint (idempotent)
do $$ begin
...
insert into public.posts (id, user_id, type, content, image_url, video_url, topics,
  ...
  ('b0000000-0000-4000-8000-000000000003', 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a08', 'video',
   ...
   null, 'https://www.youtube.com/watch?v=dQw4w9WgXcQ', '{Recovery,Fitness}', 356, 0, 44, 61, 149, 1750, now() - interval '9 hours'),
  ...
  -- VIDEO POST REMOVED: ('b0000000-0000-4000-8000-000000000008', dad song video)
  ...
  -- VIDEO POST REMOVED: ('b0000000-0000-4000-8000-000000000013', morning vlog video)
  ...
  -- VIDEO POST REMOVED: ('b0000000-0000-4000-8000-000000000016', grounding technique video)
on conflict (id) do nothing;
