...
update public.profiles
   set bio = case username
       ...
       when 'lina.reads'     then '41 books last year. Ask me for a rec, I dare you.'
...
insert into public.profiles (user_id, username, display_name, avatar_hue, bio, location, verified, is_persona) values
  ...
  (null, 'lina.reads',     'Lina Petrova',  265, '41 books last year. Ask me for a rec, I dare you.', 'Sofia', false, true),
...
select p.id, v.title, v.description, v.category_id, v.duration_days, v.difficulty, v.daily_task, v.cover_url, v.tags,
  ...
  from (values
    ...
    ('lina.reads',     '30 Days Reading',           'Twenty pages or twenty minutes a day, whichever comes first. Fiction, non-fiction, papers — reading is reading.', 'reading', 30, 'easy', 'Read 20+ pages and write one sentence about them.', '/images/covers/reading.jpg', '{books,focus,habits}', 1755, 903, 465, 97, 11800, 712, 47),
    ...
    ('lina.reads',     '14 Days Journaling',        'One page, longhand, every night. No structure required — brain dump, gratitude list, or a rerun of the day.', 'mindfulness', 14, 'easy', 'Write one journal page and note the prompt or topic.', null, '{writing,reflection,evening}', 760, 355, 199, 33, 4300, 341, 30),
...
select c.id, p.id, v.body, now() - (v.age_days || ' days')::interval
  from (values
    ...
    ('30 Days Coding', 'lina.reads', 'Joined yesterday — pairing this with the reading challenge keeps evenings screen-light.', 2),
